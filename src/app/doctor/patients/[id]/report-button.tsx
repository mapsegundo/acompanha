    ...
    const generateReport = async () => {
        ...
        try {
            ...
            autoTable(doc, {
                ...
                body: [
                    ['Qualidade do Sono', avgSono, Number(avgSono) >= 7 ? '[OK] Bom' : Number(avgSono) >= 5 ? '[!] Atencao' : '[X] Critico'],
                    ['Cansaço', avgCansaco, Number(avgCansaco) <= 4 ? '[OK] Bom' : Number(avgCansaco) <= 7 ? '[!] Atencao' : '[X] Critico'],
                    ['Estresse', avgEstresse, Number(avgEstresse) <= 4 ? '[OK] Bom' : Number(avgEstresse) <= 7 ? '[!] Atencao' : '[X] Critico'],
                    ['Humor', avgHumor, Number(avgHumor) >= 7 ? '[OK] Bom' : Number(avgHumor) >= 5 ? '[!] Atencao' : '[X] Critico'],
                    ['Dor Muscular', avgDor, Number(avgDor) <= 4 ? '[OK] Bom' : Number(avgDor) <= 7 ? '[!] Atencao' : '[X] Critico'],
                    ['Libido', avgLibido, Number(avgLibido) >= 6 ? '[OK] Bom' : Number(avgLibido) >= 4 ? '[!] Atencao' : '[X] Critico'],
                    ['Horas Treino (semana)', avgTreino, '-'],
                    ['Lesões Recentes', `${lesoes}/${recentCheckins.length}`, lesoes === 0 ? '[OK] Nenhuma' : '[!] Atencao'],
                ],
                theme: 'striped',
