...
export function PatientSummary({ checkins }: PatientSummaryProps) {
  ...
  const metrics = [
    ...
    { key: "peso", label: "Peso", higherIsBetter: null },
  ] as const
  ...
  const changes = metrics.map((metric) => {
    ...
    if (metric.higherIsBetter !== null) {
      ...
      else if (metric.higherIsBetter) status = diff > 0 ? "improved" : "worsened"
      else status = diff < 0 ? "improved" : "worsened"
    } else {
      status = "neutral"
    }
  ...
  const improvements = changes.filter((change) => change.status === "improved")
  const worsenings = changes.filter((change) => change.status === "worsened")
  const hasImprovements = improvements.length > 0
  const hasWorsenings = worsenings.length > 0
  const hasBothSides = hasImprovements && hasWorsenings
    ...
    <Card className="border-l-4 border-l-blue-600 shadow-sm">
      ...
      <CardContent className="space-y-4">
        <div className="bg-slate-50 p-3 rounded-md text-sm text-slate-700 leading-relaxed">
          {generateSummaryText().split("**").map((part, index) =>
            index % 2 === 1 ? <strong key={index}>{part}</strong> : part
          )}
        </div>

        <div className={cn("grid grid-cols-1 gap-3", hasBothSides && "md:grid-cols-2")}>
          {hasImprovements && (
            <div className="space-y-2">
              ...
              <div className="grid gap-2">
                {improvements.map((item) => (
                  <div
                    key={item.key}
                    className="flex items-center justify-between bg-green-50/50 p-2 rounded border border-green-100"
                  >
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-2">
                {worsenings.map((item) => (
                  <div
                    key={item.key}
                    className="flex items-center justify-between bg-red-50/50 p-2 rounded border border-red-100"
                  >
                    <span className="text-sm font-medium text-slate-700">{item.label}</span>
          ...
          {!hasImprovements && !hasWorsenings && (
            <div className="bg-slate-50 p-3 rounded-md border text-sm text-slate-600">
              Sem alterações relevantes nos indicadores nesta semana.
            </div>
          )}
        </div>
      </CardContent>
